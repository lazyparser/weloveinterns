...
### NJ57 Android(AOSP) RISC-V 移植开发实习生（每个子项目2-3人）
...
1. 自我评价LV2以上。由自学和钻研技术的热情。在投递简历之前，尝试阅读或编译过 AOSP RISC-V 的代码。
...
1. 完善和优化方舟编译器中的 MapleC、MapleJS 的 RISC-V 后端。目前有一个大体成型的框架，但是还有很多的工作需要做。
2. 阅读方舟编译器的代码，理解之后进行技术报告和分享。
3. （可选）参加PLCT实验室或方舟开源社区后续的技术分享沙龙等活动。
