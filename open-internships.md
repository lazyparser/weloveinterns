...
说明事项：
- 全员远程工作，不需要坐班，也没有工位提供。
- 主要注意开源社区都是个人出道，所有的行为都是公开的，要有一颗大心脏。
- 劳务费每个月根据产出浮动，浮动区间 0k - 5k，正常出勤一般是税前 2k-3k 之间。不算高。
- mentor 默认是吴伟（lazyparser）。

### BJ90 开源社区运营助理（SG2042 and Duo）实习生（20230801开放， 共 5 名）
...
### BJ76 OpenCV RISC-V P 扩展移植实习生（进行中，名额 1）
...
### BJ73 SystemVerilog编译器工程师(slang to mlir), （进行中，名额 2）
...
### BJ15 QEMU/Spike 开源模拟器开发实习生（进行中，名额 2）
...
### BJ18 Clang/LLVM 开发实习生（进行中，名额 2）
